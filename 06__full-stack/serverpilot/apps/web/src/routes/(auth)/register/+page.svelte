<script lang="ts">
	...
	// Schema validating input formats & matches
	...
	const { form, errors, enhance } = superForm(defaults(zod4(registerSchema)), {
		SPA: true,
		...
		async onUpdate({ form }) {
			...
			loading = true;
			try {
				await authStore.register(form.data.email, form.data.password);
				toast.success('Welcome to ServerPilot! Your account is active.');
				goto('/dashboard');
			} catch (err: any) {
				toast.error(err.message || 'Failed to create your account.');
			} finally {
				loading = false;
			}
		}
...
<svelte:head>
	<title>Register | ServerPilot</title>
	<meta name="description" content="Create a new ServerPilot account to manage infrastructure." />
</svelte:head>
...
<div class="relative flex min-h-screen items-center justify-center overflow-hidden bg-zinc-950 p-4">
	...
	<div class="relative z-10 w-full max-w-md">
		...
		<Card
			...
			description="Start monitoring your servers. Standard setup takes less than a minute."
		>
			<form use:enhance method="POST" class="flex flex-col gap-4">
				...
				<Button type="submit" class="mt-3 w-full" {loading}>Create Account</Button>

				<!-- Switch login trigger -->
