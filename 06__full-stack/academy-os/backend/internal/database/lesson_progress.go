...
func (r *LessonProgressRepository) UpsertProgress(
	...
	err := r.db.QueryRow(
		ctx,
		`
		INSERT INTO lesson_progress (id, academy_id, user_id, lesson_id, position_seconds, updated_at)
		VALUES ($1, $2, $3, $4, $5, NOW())
		ON CONFLICT (academy_id, user_id, lesson_id)
		DO UPDATE SET
			position_seconds = EXCLUDED.position_seconds,
			updated_at = NOW()
		RETURNING id, academy_id, user_id, lesson_id, position_seconds, completed_at, updated_at
		`,
		id, academyID, userID, lessonID, positionSeconds,
	).Scan(&rec.ID, &rec.AcademyID, &rec.UserID, &rec.LessonID, &rec.PositionSeconds, &rec.CompletedAt, &rec.UpdatedAt)
...
func (r *LessonProgressRepository) MarkComplete(
	...
	err := r.db.QueryRow(
		ctx,
		`
		INSERT INTO lesson_progress (id, academy_id, user_id, lesson_id, completed_at, updated_at)
...
func (r *LessonProgressRepository) GetProgress(
	...
	err := r.db.QueryRow(
		ctx,
		`
		SELECT id, academy_id, user_id, lesson_id, position_seconds, completed_at, updated_at
		FROM lesson_progress
		WHERE academy_id = $1
		  AND user_id = $2
		  AND lesson_id = $3
		`,
		academyID, userID, lessonID,
	).Scan(&rec.ID, &rec.AcademyID, &rec.UserID, &rec.LessonID, &rec.PositionSeconds, &rec.CompletedAt, &rec.UpdatedAt)
...
func (r *LessonProgressRepository) GetProgressForCourse(
	...
	rows, err := r.db.Query(
		ctx,
		`
		SELECT lp.id, lp.academy_id, lp.user_id, lp.lesson_id, lp.position_seconds, lp.completed_at, lp.updated_at
		FROM lesson_progress lp
		JOIN lessons l ON l.id = lp.lesson_id
		JOIN sections s ON s.id = l.section_id
		WHERE lp.academy_id = $1
		  AND lp.user_id = $2
		  AND s.course_id = $3
		`,
	...
	for rows.Next() {
		var rec LessonProgressRecord
		if err := rows.Scan(&rec.ID, &rec.AcademyID, &rec.UserID, &rec.LessonID, &rec.PositionSeconds, &rec.CompletedAt, &rec.UpdatedAt); err != nil {
