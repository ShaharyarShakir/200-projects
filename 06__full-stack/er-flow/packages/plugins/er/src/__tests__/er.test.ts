...
describe("ER Logical Engine", () => {
  ...
  it("should contain complete starter templates", () => {
    const blogTemplate = ER_TEMPLATES.find(t => t.id === "blog");
    const ecommerceTemplate = ER_TEMPLATES.find(t => t.id === "ecommerce");
    expect(blogTemplate?.entities.length).toBeGreaterThan(0);
    expect(ecommerceTemplate?.entities.length).toBeGreaterThan(0);
  });
});
