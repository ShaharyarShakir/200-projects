...
import { calculateSnapping } from "@eraser/graph-engine";
import {
  isPointInRect,
  isPointInCircle,
  isPointInDiamond,
        ...
        if (selectedShapes.length === 1) {
          ...
          if (startGeo) {
            if (s.type === "arrow" || s.type === "line") {
              ...
              let targetX = startGeo.x + dx;
              let targetY = startGeo.y + dy;

              // Grid snapping (20px interval)
              ...
              // Guides Snapping
              const otherNodes = engine.getShapes().filter(
                (shape) =>
                  shape.id !== s.id &&
                  shape.type !== "arrow" &&
                  shape.type !== "line" &&
              ...
              const snapResult = calculateSnapping(
                s.id,
                { x: targetX, y: targetY, width: s.width, height: s.height },
                otherNodes
              );
              ...
              engine.activeGuides = snapResult.guides;
            }
          }
        } else {
          // Multiple shapes delta dragging
          for (const s of selectedShapes) {
            const startGeo = this.startGeometries.get(s.id);
