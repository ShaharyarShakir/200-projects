...
class ELDGenerateSerializer(serializers.Serializer):
    ...
    cycle_used = serializers.FloatField(
        required=False, min_value=0.0, max_value=70.0, default=0.0
    )
    start_time = serializers.DateTimeField(required=False, allow_null=True)
    driver_name = serializers.CharField(
        required=False, allow_blank=True, default="John Doe"
    )
    carrier = serializers.CharField(
        required=False, allow_blank=True, default="Apex Logistics Inc."
    )
    vehicle_number = serializers.CharField(
        required=False, allow_blank=True, default="Truck #402"
    )
    trailer_number = serializers.CharField(
        required=False, allow_blank=True, default="TR-881"
    )
