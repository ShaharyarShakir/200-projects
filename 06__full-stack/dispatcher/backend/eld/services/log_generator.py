...
class ELDLogGenerator:
    ...
    def generate_daily_logs(
        ...
        curr_day_start = datetime(first_dt.year, first_dt.month, first_dt.day, 0, 0, 0, tzinfo=timezone.utc)
        final_day_end = datetime(last_dt.year, last_dt.month, last_dt.day, 0, 0, 0, tzinfo=timezone.utc) + timedelta(days=1)
        ...
        while curr_day_start < final_day_end:
            ...
            if persist and trip:
                ...
                duty_event_objs = []
                for seg in padded_segments:
                    duty_event_objs.append(DutyEvent(
                        daily_log=log_obj,
                        type=seg["status"],
