...
def draw_fmcsa_grid(graph_data: List[Dict[str, Any]], width: float = 520, height: float = 140) -> Drawing:
    """
    ...
    d.add(Rect(chart_x, chart_y, chart_w, chart_h, fillColor=colors.HexColor("#f8fafc"), strokeColor=colors.HexColor("#64748b"), strokeWidth=1))
    ...
    for i, lbl in enumerate(labels):
        ...
        d.add(String(10, y_center - 4, lbl, fontName="Helvetica-Bold", fontSize=8, fillColor=colors.HexColor("#1e293b")))
        # Horizontal row divider line
        if i > 0:
            y_div = chart_y + chart_h - i * row_h
            d.add(Line(chart_x, y_div, chart_x + chart_w, y_div, strokeColor=colors.HexColor("#cbd5e1"), strokeWidth=0.5))

    # 24 Hour columns & ticks
    for hr in range(25):
        x = chart_x + (hr / 24.0) * chart_w
        # Major vertical line
        d.add(Line(x, chart_y, x, chart_y + chart_h, strokeColor=colors.HexColor("#e2e8f0" if hr % 2 != 0 else "#94a3b8"), strokeWidth=0.5 if hr % 2 != 0 else 1))

        # Hour numbers
        if hr % 2 == 0 or hr == 24:
            hr_str = "M" if hr in (0, 24) else ("N" if hr == 12 else str(hr))
            d.add(String(x - 3, chart_y + chart_h + 4, hr_str, fontName="Helvetica", fontSize=7, fillColor=colors.HexColor("#475569")))
            d.add(String(x - 3, chart_y - 10, hr_str, fontName="Helvetica", fontSize=7, fillColor=colors.HexColor("#475569")))

        # Half-hour tick marks
        if hr < 24:
            x_half = chart_x + ((hr + 0.5) / 24.0) * chart_w
            d.add(Line(x_half, chart_y + chart_h - 4, x_half, chart_y + chart_h, strokeColor=colors.HexColor("#cbd5e1"), strokeWidth=0.5))
            d.add(Line(x_half, chart_y, x_half, chart_y + 4, strokeColor=colors.HexColor("#cbd5e1"), strokeWidth=0.5))

    # Totals column label
    d.add(String(chart_x + chart_w + 8, chart_y + chart_h + 4, "HOURS", fontName="Helvetica-Bold", fontSize=7, fillColor=colors.HexColor("#475569")))

    # Calculate row totals
    row_totals = {DutyStatus.OFF_DUTY: 0.0, DutyStatus.SLEEPER_BERTH: 0.0, DutyStatus.DRIVING: 0.0, DutyStatus.ON_DUTY: 0.0}
    for seg in graph_data:
        st = seg["status"]
        if st in row_totals:
            row_totals[st] += seg["duration"]

    for i, st in enumerate([DutyStatus.OFF_DUTY, DutyStatus.SLEEPER_BERTH, DutyStatus.DRIVING, DutyStatus.ON_DUTY]):
        y_center = chart_y + chart_h - (i + 0.5) * row_h
        tot_str = f"{row_totals[st]:.1f}"
        d.add(String(chart_x + chart_w + 10, y_center - 4, tot_str, fontName="Helvetica-Bold", fontSize=8, fillColor=colors.HexColor("#0f172a")))
    ...
    for seg in graph_data:
        ...
        if prev_x is not None and prev_y is not None:
            ...
            d.add(Line(prev_x, prev_y, x1, y, strokeColor=colors.HexColor("#2563eb"), strokeWidth=2.5))

        # Horizontal status line
        d.add(Line(x1, y, x2, y, strokeColor=colors.HexColor("#2563eb"), strokeWidth=2.5))
        ...
        for idx, log in enumerate(daily_logs):
            ...
            header_table_data = [
                [
                    ...
                    Paragraph(f"DAY #{log.get('day_number')} &nbsp;|&nbsp; DATE: <b>{log.get('date')}</b>", subtitle_style)
                ]
            ]
            ...
            info_data = [
                ...
                [
                    ...
                    Paragraph(str(d_info.get("home_terminal", "N/A")), header_val_style)
                ],
                [
                ...
                [
                    Paragraph(str(d_info.get("vehicle_number", "N/A")), header_val_style),
                    Paragraph(str(d_info.get("trailer_number", "N/A")), header_val_style),
            ...
            grid_drawing = draw_fmcsa_grid(log.get("graph_data", []), width=540, height=135)
            ...
            summary_data = [
                ...
                [
                    Paragraph(f"<b>{summary.get('total_distance', 0)} mi</b>", header_val_style),
                    Paragraph(f"<b>{summary.get('driving_hours', 0)} h</b>", header_val_style),
                    Paragraph(f"<b>{summary.get('duty_hours', 0)} h</b>", header_val_style),
                    Paragraph(f"<b>{summary.get('sleeper_hours', 0)} h</b>", header_val_style),
                    Paragraph(f"<b>{summary.get('off_duty_hours', 0)} h</b>", header_val_style),
                    Paragraph(f"<b>{summary.get('cycle_used', 0)} h</b>", header_val_style)
                ]
            ]
            ...
            for ev in log.get("duty_events", []):
                st_time = ev.get("start_time", "").split("T")[-1][:5] if "T" in ev.get("start_time", "") else ev.get("start_time", "")
                end_time = ev.get("end_time", "").split("T")[-1][:5] if "T" in ev.get("end_time", "") else ev.get("end_time", "")
            ...
            sig_data = [
                [
                    Paragraph("I CERTIFY THAT THESE ENTRIES ARE TRUE AND CORRECT:", header_label_style),
                    Paragraph("DRIVER SIGNATURE: _______________________", header_label_style),
