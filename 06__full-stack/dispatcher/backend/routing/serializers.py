    ...
    if isinstance(value, dict):
        ...
        if lat is None or lng is None:
            raise serializers.ValidationError(
                "Location object must contain 'lat' and 'lng' properties."
            )
        value = [lng, lat]

    if not isinstance(value, (list, tuple)) or len(value) != 2:
        raise serializers.ValidationError(
            "Coordinate must be a list of 2 numbers [longitude, latitude] or object with {lat, lng}."
        )
...
class RouteRequestSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not origin_val:
            raise serializers.ValidationError(
                {"current": "Either 'current' or 'origin' coordinate is required."}
            )
