...
KNOWN_CITIES = {
    "lahore": {
        ...
        "lat": 31.5204,
        "lng": 74.3587,
        "place_id": "known_lahore",
    },
    "islamabad": {
        "display_name": "Islamabad, ICT, Pakistan",
        "lat": 33.6844,
        "lng": 73.0479,
        "place_id": "known_islamabad",
    },
    "karachi": {
        "display_name": "Karachi, Sindh, Pakistan",
        "lat": 24.8607,
        "lng": 67.0011,
        "place_id": "known_karachi",
    },
    "rawalpindi": {
        "display_name": "Rawalpindi, Punjab, Pakistan",
        "lat": 33.5651,
        "lng": 73.0169,
        "place_id": "known_rawalpindi",
    },
    "multan": {
        "display_name": "Multan, Punjab, Pakistan",
        "lat": 30.1575,
        "lng": 71.5249,
        "place_id": "known_multan",
    },
    "faisalabad": {
        "display_name": "Faisalabad, Punjab, Pakistan",
        "lat": 31.4504,
        "lng": 73.1350,
        "place_id": "known_faisalabad",
    },
    "chicago": {
        "display_name": "Chicago, Illinois, United States",
        "lat": 41.8781,
        "lng": -87.6298,
        "place_id": "known_chicago",
    },
    "new york": {
...
class GeocodingService:
    ...
    def reverse(lat: float, lng: float) -> dict:
        ...
        try:
                ...
                if resp.status_code == 200:
                    ...
                    result = {
                        "display_name": data.get(
                            "display_name", f"{rounded_lat}, {rounded_lng}"
                        ),
                        "lat": rounded_lat,
                        "lng": rounded_lng,
            ...
            logger.warning(
                f"Nominatim reverse geocode failed for {rounded_lat},{rounded_lng}: {e}"
            )

        # Fallback to nearest known city or coordinates string
        for city_name, data in KNOWN_CITIES.items():
            if (
                abs(data["lat"] - rounded_lat) < 0.3
                and abs(data["lng"] - rounded_lng) < 0.3
            ):
                return data
