...
class ScheduleEngine:
    ...
    def generate_schedule(
        ...
        def add_event(
            event_type: str, hours: float, dist: float, location: str, notes: str
        ):
        ...
        add_event(
            ...
            0.0,
            pickup_name,
            "Pickup cargo & pre-trip inspection",
        )
        ...
        while remaining_drive_hours > 0.001:
            ...
            break_cap = max(
                0.0, self.config.BREAK_AFTER_DRIVE_HOURS - drive_since_break
            )
            sleep_drive_cap = max(
                0.0, self.config.MAX_DRIVING_HOURS - drive_since_sleep
            )
            ...
            if allowed_drive <= 0.001:
                if sleep_drive_cap <= 0.001 or sleep_duty_cap <= 0.001:
                    ...
                    add_event(
                        ...
                        0.0,
                        "Rest Area / Sleeper",
                        "10-Hour Mandatory Sleeper Berth Rest",
                    )
                    drive_since_sleep = 0.0
                    duty_since_sleep = 0.0
                    drive_since_break = 0.0
                    ...
                    add_event(
                        ...
                        0.0,
                        "Rest Area",
                        "30-Minute Mandatory Driving Break",
                    )
            ...
            if (
                abs(dist_covered - next_fuel_milestone) < 0.1
                or dist_covered > next_fuel_milestone
            ):
                ...
                add_event(
                    ...
                    fuel_dur,
                    0.0,
                    f"Fuel Station (Mile {round(dist_covered, 1)})",
                    "Refuel & 30-min Stop",
                )
            ...
            if (
                drive_since_sleep >= self.config.MAX_DRIVING_HOURS - 0.001
                or duty_since_sleep >= self.config.MAX_DUTY_HOURS - 0.001
            ):
        ...
        add_event(
            ...
            0.0,
            dropoff_name,
            "Unload cargo & post-trip inspection",
        )

        # Driver remaining status after trip
        status = self.hos_engine.calculate_remaining_hours(
            drive_since_sleep, duty_since_sleep, drive_since_break, current_cycle_used
        )
        ...
        return {
            ...
            "total_trip_elapsed_hours": round(
                (curr_time - start_datetime).total_seconds() / 3600.0, 2
            ),
