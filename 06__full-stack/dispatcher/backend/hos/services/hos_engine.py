...
class HOSEngine:
    ...
    def calculate_remaining_hours(
        ...
        """
        remaining_drive = max(0.0, round(self.config.MAX_DRIVING_HOURS - drive_since_sleep, 2))
        remaining_duty = max(0.0, round(self.config.MAX_DUTY_HOURS - duty_since_sleep, 2))
        remaining_until_break = max(0.0, round(self.config.BREAK_AFTER_DRIVE_HOURS - drive_since_break, 2))
        ...
        max_allowed_drive_now = min(remaining_drive, remaining_duty, remaining_until_break, remaining_cycle)
