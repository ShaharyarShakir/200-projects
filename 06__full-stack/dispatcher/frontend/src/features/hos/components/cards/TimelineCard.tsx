...
export const TimelineCard: React.FC<TimelineCardProps> = ({ event, isFirst, isLast }) => {
  if (event.type === 'fuel') {
  ...
  return (
    <div className="bg-neutral-50/90 border border-neutral-200 rounded-xl p-4 flex items-center justify-between shadow-sm">
      <div className="flex items-center gap-3">
        <DriverStatusBadge type={event.type} />
        <div>
          <p className="font-semibold text-sm text-neutral-800 flex items-center gap-1">
            {event.notes}
          </p>
          <p className="text-xs text-neutral-400 mt-0.5 flex items-center gap-1">
            <MapPin className="w-3.5 h-3.5 text-brand-600" /> {event.location}
            {event.distance > 0 && <span className="font-mono text-neutral-500">({event.distance} miles)</span>}
          </p>
        </div>
      </div>

      <div className="text-right font-mono">
        <span className="text-sm font-bold text-neutral-900">{startTime} - {endTime}</span>
