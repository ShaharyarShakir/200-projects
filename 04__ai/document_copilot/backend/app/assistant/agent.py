...
def run_document_agent(query: str, deps: DocumentAgentDeps) -> GroundedAnswer:
    emit_agent_start(
        ...
        request_limit=settings.google_agent_request_limit,
    )
    result = get_document_agent().run_sync(
        query,
        deps=deps,
        usage_limits=UsageLimits(request_limit=settings.google_agent_request_limit),
    )
    usage = result.usage
    emit_agent_done(
        deps,
